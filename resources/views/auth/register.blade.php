...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'Task Manager') }} - Register</title>
    ...
    <body class="font-sans antialiased bg-[#1E293B] h-screen overflow-hidden flex items-center justify-center p-4 relative">
        ...
        <div class="w-full max-w-md my-auto flex flex-col justify-center max-h-full relative z-10">
            ...
            <!-- Register Card -->
            <div class="bg-white rounded-2xl shadow-xl border border-slate-100 p-6 md:px-8 md:py-6 overflow-y-auto max-h-[calc(100vh-120px)] no-scrollbar">
                <h2 class="text-xl font-bold text-slate-800 mb-0.5">Create an account</h2>
                <p class="text-slate-500 mb-4 text-xs md:text-sm">Join us to organize your tasks with AI help</p>
                ...
                <form method="POST" action="{{ route('register') }}" class="space-y-3.5">
                    ...
                    <div>
                        ...
                        <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name"
                            class="w-full bg-slate-50 border @error('name') border-red-300 focus:ring-red-500 focus:border-red-500 @else border-slate-200 focus:ring-blue-500 focus:border-blue-500 @enderror text-slate-800 rounded-xl p-2.5 md:p-3 focus:ring-2 focus:outline-none transition-all placeholder-slate-400 text-xs md:text-sm"
                            placeholder="John Doe">
                        @error('name')
                            <p class="text-red-600 text-[10px] md:text-xs mt-1 font-medium flex items-center gap-1">
                    ...
                    <div>
                        ...
                        <input id="email" type="email" name="email" value="{{ old('email') }}" required autocomplete="username"
                            class="w-full bg-slate-50 border @error('email') border-red-300 focus:ring-red-500 focus:border-red-500 @else border-slate-200 focus:ring-blue-500 focus:border-blue-500 @enderror text-slate-800 rounded-xl p-2.5 md:p-3 focus:ring-2 focus:outline-none transition-all placeholder-slate-400 text-xs md:text-sm"
                    ...
                    <div>
                        ...
                        <input id="password" type="password" name="password" required autocomplete="new-password"
                            class="w-full bg-slate-50 border @error('password') border-red-300 focus:ring-red-500 focus:border-red-500 @else border-slate-200 focus:ring-blue-500 focus:border-blue-500 @enderror text-slate-800 rounded-xl p-2.5 md:p-3 focus:ring-2 focus:outline-none transition-all placeholder-slate-400 text-xs md:text-sm"
                            placeholder="••••••••">
                        @error('password')
                            <p class="text-red-600 text-[10px] md:text-xs mt-1 font-medium flex items-center gap-1">
                                <svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                    ...
                    <div>
                        ...
                        <input id="password_confirmation" type="password" name="password_confirmation" required autocomplete="new-password"
                            class="w-full bg-slate-50 border @error('password_confirmation') border-red-300 focus:ring-red-500 focus:border-red-500 @else border-slate-200 focus:ring-blue-500 focus:border-blue-500 @enderror text-slate-800 rounded-xl p-2.5 md:p-3 focus:ring-2 focus:outline-none transition-all placeholder-slate-400 text-xs md:text-sm"
                            placeholder="••••••••">
                    ...
                    <!-- Submit Button -->
                    <button type="submit"
                        class="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 rounded-xl shadow-lg shadow-blue-500/20 hover:shadow-blue-500/30 transition-all active:scale-[0.98] transform text-xs md:text-sm cursor-pointer mt-3">
                        Register
                    </button>
                </form>

                <p class="text-center text-xs md:text-sm text-slate-500 mt-4 font-medium">
                    Already registered?
                    <a href="{{ route('login') }}" class="font-bold text-blue-500 hover:text-blue-600 transition-colors">Sign In</a>
                </p>
            </div>
        </div>
